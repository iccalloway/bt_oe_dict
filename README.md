...
```
resources/bt_text.txt  ──src/parse.py──►  parsed/*.json  ──src/write_xml.py──►  oe_templates/MyDictionary.xml  ──compile.sh──►  installed .dictionary
```

### 1. Parse — `src/parse.py` + `src/llm.py` + `src/schema.py`

- `parse.py` splits `resources/bt_text.txt` on blank lines, shuffles, and iterates entries.
...
```bash
uv run src/parse.py                        # populate parsed/ (skips existing)
uv run src/write_xml.py                    # regenerate oe_templates/MyDictionary.xml
./compile.sh                               # compile and install the .dictionary
```

To flag Old English words whose transcription still has ambiguous ⟨c⟩/⟨g⟩ (unresolved K/G) and manually correct them:

```bash
uv run src/velar_normalization/find_ambiguous.py   # writes src/velar_normalization/corrections.csv
```

Then hand-edit column B of that CSV — usually by adding hyphens (e.g. `þeōdendlīc → þeōdend-līc`) so the morpheme lookup in `src/velar_normalization/morphemes.json` fires. `write_xml.py` uses the corrected form as the headword and preserves the original as a search alias.
...
| `resources/template.xml` | Empty `<d:dictionary>` scaffold that `write_xml.py` populates |
| `src/schema.py` | Pydantic models — `EntryBatch`, `OldEnglishEntry`, `Definition`, `SubSense`, POS variants, `Quotation` |
| `src/llm.py` | Thin wrapper around `anthropic.messages.create` with forced tool call + prompt caching |
| `src/parse.py` | LLM parsing loop with skip-if-already-parsed, merge-on-collision, and failure collection |
| `src/phonology.py` | Old English → IPA transcriber (diphthongs, palatalization, morpheme overrides) |
| `src/write_xml.py` | JSON → Apple Dictionary XML renderer |
| `src/velar_normalization/morphemes.json` | Whole-morpheme phoneme overrides for resolving ambiguous ⟨c⟩/⟨g⟩ |
| `src/velar_normalization/find_ambiguous.py` | Lists unresolved K/G words and writes `corrections.csv` |
| `src/velar_normalization/corrections.csv` | Hand-edited citation-form corrections (adds hyphens to expose morphemes) |
| `oe_templates/` | DDK project (Makefile, `MyDictionary.css`, `MyInfo.plist`); receives the generated `MyDictionary.xml` |
...
- Scripts resolve paths relative to the project root (via `Path(__file__).parent.parent`), so they can be invoked from any working directory.
