...
class AffixPOS(_Strict):
    """Bound morpheme — an affix that only appears attached to a stem, never as
    a free word. Use this category (not noun/verb/adverb) whenever the citation
    form is flanked by a hyphen: leading hyphen → suffix (`-icge`, `-el`, `-estre`),
    trailing hyphen → prefix (`ge-`, `of-`, `æt-`). Their glosses describe what
    they do to other words ('a suffix denoting a female agent', 'a prefix that
    intensifies the verb'), not something they name in the world."""

    category: Literal["affix"] = "affix"
    kind: Literal["prefix", "suffix"] = Field(
        description="'prefix' if the citation form has a trailing hyphen (ge-, of-, æt-); "
                    "'suffix' if it has a leading hyphen (-icge, -el, -estre)."
    )
...
POS = Annotated[
    Union[NounPOS, VerbPOS, AdjectivePOS, ConjunctionPOS, AdverbPOS, ParticiplePOS, PrepositionPOS, PronounPOS, InterjectionPOS, AffixPOS],
