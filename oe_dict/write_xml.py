...
if __name__ == "__main__":
    ...
    for file in sorted(glob.glob(str(Path(__file__).parent / "parsed_entries" / "*.json"))):
        with open(file, "r", encoding="utf-8") as f:
            d = json.load(f)
            entry = create_entry(d, seen_aliases)
            if entry is None:
                continue
            # Two source JSONs can collide on the same normalized citation
            # (e.g. `bríd-gifu` and `brīd-gifu` both → `brīd-gifu` under
            # _ACUTE_TO_MACRON). Suffix duplicates so DDK's replace_entryid
            # pass doesn't abort with "Duplicate entry_id".
            base_id = entry.attrib["id"]
            new_id = base_id
            n = 2
            ...
            seen_ids.add(new_id)
            if new_id != base_id:
                entry.attrib["id"] = new_id
            root.append(entry)

    with open(_ROOT / "oe_templates" / "MyDictionary.oe.xml", "wb") as g:
